...
<?php
	...
	echo "<TABLE>";
		...
		echo "<TR>";
		echo "<TH width='450px' align='center'></TH>";
		for($j=0;$j<$i;$j++){
		...
		for($j=0;$j<$i;$j++){
			if((float)$vd1[$j] > 0)
				echo "<TD align='center' class='fb' style='color:blue'>".number_format((float)$vd1[$j], 2, '.', ',')."</TD>";
			else
				echo "<TD align='center' class='fb' style='color:red'>".number_format((float)$vd1[$j], 2, '.', ',')."</TD>";
		}
		echo "</TR>";
